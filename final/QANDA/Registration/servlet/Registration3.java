...
// SvlEx2::doGet()
	...
	out.println(" <link rel=\"stylesheet\" href=\"../css/common.css\">" );         /* css ファイルの読み込み(このファイルの位置からcssへのパス) */
	...
	if(strUserName.equals("") || strPassword.equals("")) {
		...
		if(row_count != 0){
			...
			out.println("アカウントが登録されました．");
			out.println("<button type=“button” onclick=\"location.href='../../login/Login.html'\">ログイン画面へ</button>");
		    }
		    // 普通はここに落ちることはない
		    else {
			out.println("</header><main>");         /* header ここまで, mainここから -------------------------------  */
			out.println("何らかの理由でアカウントが登録されませんでした．");
			//  out.println("<button type=“button” onclick=\"location.href='../Registration.html'\">アカウント作成画面へ画面へ</button>");
		    }  
		}
